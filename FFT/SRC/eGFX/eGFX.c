...
void eGFX_BitStreamPut(uint8_t * BitStream, uint32_t Position, uint32_t PixelState)
{
...
uint32_t eGFX_BitStreamGet(uint8_t * BitStream, uint32_t Position)
{
...
uint32_t eGFX_AnimatorsAreComplete()
{
	uint32_t RetVal = TRUE;
	uint32_t i;
...
void eGFX_PauseAnimator(void * A)
{
	((eGFX_AnimatorHeader *)(A))->State = eGFX_OBJECT_STATE_INACTIVE;
}

void eGFX_StartAnimator(void * A)
{
